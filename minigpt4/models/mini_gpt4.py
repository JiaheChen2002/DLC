        ...
        if prompt_cd is not None:
            if image is not None:
                ...
                if isinstance(prompt_cd, str):
                    prompt_cd = [prompt_cd] * batch_size
                else:
                    assert len(prompt_cd) == batch_size, "The number of prompts must be equal to the batch size."
                    
                # 获取对比解码的嵌入
                img_embeds_icd, atts_img_icd = self.encode_img(image)
                inputs_embeds_cd, attention_mask_cd, _ = self.prompt_wrap(img_embeds_icd, atts_img_icd, prompt_cd)
                
                # 添加 BOS token
                bos = torch.ones([batch_size, 1],
                                dtype=torch.int64,
                                device=inputs_embeds_cd.device) * self.llama_tokenizer.bos_token_id
                bos_embeds = self.embed_tokens(bos)
                atts_bos_cd = attention_mask_cd[:, :1]
                
                inputs_embeds_cd = torch.cat([bos_embeds, inputs_embeds_cd], dim=1)
                # attention_mask_cd = torch.cat([atts_bos_cd, attention_mask_cd], dim=1)
